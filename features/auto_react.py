...
class AutoReact(discord.Cog):
    ...
    async def list_auto_reacts(self, ctx: discord.ApplicationContext):
        try:
            ...
            for i in all_settings:
                ...
                current_group_msg += trl(ctx.user.id, ctx.guild.id, "auto_react_list_row").format(id=str(i['_id']),
                                                                                                  trigger=i[
                                                                                                      'TriggerText'],
                                                                                                  emoji=i['Emoji'])
                current_group += 1
    ...
    async def edit_trigger_auto_react(self, ctx: discord.ApplicationContext, id: str, new_trigger: str):
        try:
            ...
            edited = client['AutoReact'].update_one({'_id': ObjectId(id), 'GuildID': str(ctx.guild.id)},
